...
export default function SelectFresher( { freshers, handleClick } ) {
  ...
  return (
    <div className={styles.select}>
        <label htmlFor="freshers">Select your Fresher!</label>
        <select name="freshers" id="selectFresher" onChange={handleChange} >
            <option></option>
            {freshers.map( fresher => (
              <option value={JSON.stringify(fresher)} key={fresher.id}> {fresher.name} - {fresher.id}</option>
            ) )}
        </select>
        <Button onClick={() => handleClick(fresher)}>
          CHOOSE
        </Button>
    </div>
  )
